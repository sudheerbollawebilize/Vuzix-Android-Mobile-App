package com.webilize.vuzixfilemanager.fragments;

import android.app.Activity;
import android.content.Intent;
import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.databinding.DataBindingUtil;

import com.google.android.material.bottomsheet.BottomSheetDialogFragment;
import com.webilize.vuzixfilemanager.R;
import com.webilize.vuzixfilemanager.databinding.FragmentBladeOptionsBinding;
import com.webilize.vuzixfilemanager.models.BladeItem;
import com.webilize.vuzixfilemanager.utils.StaticUtils;
import com.webilize.vuzixfilemanager.utils.transferutils.CommunicationProtocol;

public class BladeOptionsBottomDialogFragment extends BottomSheetDialogFragment implements View.OnClickListener {
    private FragmentBladeOptionsBinding fragmentFileOptionsBinding;
    private BladeItem bladeItem;
    private CommunicationProtocol cp;

    public static BladeOptionsBottomDialogFragment newInstance(BladeItem bladeItem) {
        BladeOptionsBottomDialogFragment fileOptionsBottomDialogFragment = new BladeOptionsBottomDialogFragment();
        Bundle bundle = new Bundle();
        bundle.putParcelable("bladeItem", bladeItem);
        fileOptionsBottomDialogFragment.setArguments(bundle);
        return fileOptionsBottomDialogFragment;
    }

    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            if (getArguments().containsKey("bladeItem"))
                bladeItem = getArguments().getParcelable("bladeItem");
        }
        cp = CommunicationProtocol.getInstance();
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        fragmentFileOptionsBinding = DataBindingUtil.inflate(inflater, R.layout.fragment_blade_options, container, false);
        initComponents();
        return fragmentFileOptionsBinding.getRoot();
    }

    private void initComponents() {
        if (cp.isConnected()) {
            fragmentFileOptionsBinding.txtSaveToPhone.setEnabled(true);
            if (bladeItem.isFolder) {
                if (StaticUtils.isInDeviceFavourites(getActivity(), bladeItem.path)) {
                    fragmentFileOptionsBinding.txtBookMark.setVisibility(View.GONE);
                    fragmentFileOptionsBinding.txtRemoveBookMark.setVisibility(View.VISIBLE);
                } else {
                    fragmentFileOptionsBinding.txtBookMark.setVisibility(View.VISIBLE);
                    fragmentFileOptionsBinding.txtRemoveBookMark.setVisibility(View.GONE);
                }
            } else {
                fragmentFileOptionsBinding.txtBookMark.setVisibility(View.GONE);
                fragmentFileOptionsBinding.txtRemoveBookMark.setVisibility(View.GONE);
            }
            fragmentFileOptionsBinding.txtSaveToPhone.setOnClickListener(this);
            fragmentFileOptionsBinding.txtBookMark.setOnClickListener(this);
            fragmentFileOptionsBinding.txtRemoveBookMark.setOnClickListener(this);
        } else {
            fragmentFileOptionsBinding.txtSaveToPhone.setEnabled(false);
            fragmentFileOptionsBinding.txtBookMark.setEnabled(false);
            fragmentFileOptionsBinding.txtRemoveBookMark.setEnabled(false);
        }
    }

    @Override
    public void onClick(View view) {
        Intent intent = new Intent();
        intent.putExtra("viewid", view.getId());
        getTargetFragment().onActivityResult(getTargetRequestCode(), Activity.RESULT_OK, intent);
        dismiss();
    }

}